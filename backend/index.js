...
app.get("/api/post/:postId", async (req, res) => {
    ...
    mysqlPool.getConnection((err, con) => {
        ...
        con.query(`select * from posts LEFT JOIN user on posts.author=user.id where posts.id='${postId}'`, (err2, result, f) => {
            if (err2) {
                console.log(err2);
                return;
            }
    ...
    if (req.session.user) {
        mysqlPool.getConnection((err, con) => {
            ...
            con.query(`
               ...
               ) s ON g.group_id = s.group_id limit 10`, (err2, result, f) => {
                if (err2) {
                    console.log(err2);
                    return;
                }
...
app.post("/api/group/unsubcribe",async(req,res)=>{
    if(req.session.user){
      ...
      mysqlPool.getConnection((err, con) => {
          ...
          con.query(`delete from subscription where group_id='${req.body.group_id}' and user_id='${req.session.user.id}'`, (err2, result, f) => {
              if (err2) {
                  console.log(err2);
                  return;
              }
